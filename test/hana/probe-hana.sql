...
-- SQL_BINARY and SQL_VARBINARY are deliberately unsupported by the importer;
-- BLOB (SQL_LONGVARBINARY) above is the supported binary route. Keep this
-- boundary explicit so an unannounced change in ODBC metadata is caught.
SELECT 'import_binary_boundary=' || CASE WHEN
    NOT EXISTS (
        SELECT 1
        FROM pg_attribute AS attribute
        JOIN pg_class AS class ON class.oid = attribute.attrelid
        JOIN pg_namespace AS namespace ON namespace.oid = class.relnamespace
        WHERE namespace.nspname = 'imported'
          AND class.relname = 'ODBC_FDW_TYPE_MATRIX'
          AND attribute.attname IN ('BINARY_VALUE', 'VARBINARY_VALUE')
