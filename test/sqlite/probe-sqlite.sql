\getenv sqlite_database SQLITE_DATABASE

CREATE EXTENSION odbc_fdw;
CREATE SCHEMA probe;

CREATE SERVER sqlite FOREIGN DATA WRAPPER odbc_fdw OPTIONS (
    odbc_driver 'SQLite3', odbc_database :'sqlite_database'
);
CREATE SERVER sqlite_server_field FOREIGN DATA WRAPPER odbc_fdw OPTIONS (
    odbc_driver 'SQLite3', odbc_database :'sqlite_database',
    max_field_size '100'
);
CREATE SERVER sqlite_server_row FOREIGN DATA WRAPPER odbc_fdw OPTIONS (
    odbc_driver 'SQLite3', odbc_database :'sqlite_database',
    max_row_count '1'
);
CREATE SERVER sqlite_server_result FOREIGN DATA WRAPPER odbc_fdw OPTIONS (
    odbc_driver 'SQLite3', odbc_database :'sqlite_database',
    max_result_size '1000'
);
CREATE USER MAPPING FOR CURRENT_USER SERVER sqlite;
CREATE USER MAPPING FOR CURRENT_USER SERVER sqlite_server_field;
CREATE USER MAPPING FOR CURRENT_USER SERVER sqlite_server_row;
CREATE USER MAPPING FOR CURRENT_USER SERVER sqlite_server_result;
...
) SERVER sqlite OPTIONS (
    schema '', table 'ODBC_FDW_DATA_TYPES', id 'ID', text_value 'TEXT_VALUE',
    integer_value 'INTEGER_VALUE', decimal_value 'DECIMAL_VALUE',
    date_value 'DATE_VALUE', time_value 'TIME_VALUE',
    timestamp_value 'TIMESTAMP_VALUE', null_value 'NULL_VALUE',
    unicode_value 'UNICODE_VALUE'
);

CREATE FOREIGN TABLE probe.type_matrix (
    id integer, tiny_value smallint, small_value smallint, integer_value integer,
    bigint_value bigint, decimal_value numeric, real_value real,
    double_value double precision, boolean_value boolean, char_value text,
    varchar_value text, text_value text, date_value date, time_value time,
    timestamp_value timestamp, blob_value bytea
) SERVER sqlite OPTIONS (
    schema '', table 'ODBC_FDW_TYPE_MATRIX', id 'ID', tiny_value 'TINY_VALUE',
    small_value 'SMALL_VALUE', integer_value 'INTEGER_VALUE',
    bigint_value 'BIGINT_VALUE', decimal_value 'DECIMAL_VALUE',
    real_value 'REAL_VALUE', double_value 'DOUBLE_VALUE',
    boolean_value 'BOOLEAN_VALUE', char_value 'CHAR_VALUE',
    varchar_value 'VARCHAR_VALUE', text_value 'TEXT_VALUE',
    date_value 'DATE_VALUE', time_value 'TIME_VALUE',
    timestamp_value 'TIMESTAMP_VALUE', blob_value 'BLOB_VALUE'
);

CREATE FOREIGN TABLE probe.large_values (
    id integer, text_value text, blob_value bytea
) SERVER sqlite OPTIONS (
    schema '', table 'ODBC_FDW_LARGE_VALUES', id 'ID', text_value 'TEXT_VALUE',
    blob_value 'BLOB_VALUE'
);
CREATE FOREIGN TABLE probe.large_server_field (id integer, text_value text)
SERVER sqlite_server_field OPTIONS (
    schema '', table 'ODBC_FDW_LARGE_VALUES', id 'ID', text_value 'TEXT_VALUE'
);
CREATE FOREIGN TABLE probe.direct_server_row (id integer, text_value text)
SERVER sqlite_server_row OPTIONS (
    schema '', table 'ODBC_FDW_DATA_TYPES', id 'ID', text_value 'TEXT_VALUE'
);
CREATE FOREIGN TABLE probe.large_server_result (id integer, text_value text)
SERVER sqlite_server_result OPTIONS (
    schema '', table 'ODBC_FDW_LARGE_VALUES', id 'ID', text_value 'TEXT_VALUE'
);
CREATE FOREIGN TABLE probe.rescan_source (id integer, payload text)
SERVER sqlite OPTIONS (
    schema '', table 'ODBC_FDW_SINGLE_ROW', max_row_count '1', id 'ID', payload 'PAYLOAD'
);
CREATE FOREIGN TABLE probe.json_values (id integer, json_value jsonb)
SERVER sqlite OPTIONS (
    schema '', table 'ODBC_FDW_JSON_VALUES', id 'ID', json_value 'JSON_VALUE'
);

CREATE FOREIGN TABLE probe.case_lower (
    upper_value integer, lower_value text, mixed_value text, spaced_value text
) SERVER sqlite OPTIONS (
    schema '', table 'odbc_fdw_lower_table', upper_value 'UPPER_VALUE',
    lower_value 'lower_value', mixed_value 'MixedValue', spaced_value 'Spaced Value'
);
CREATE FOREIGN TABLE probe.case_mixed (
    upper_value integer, lower_value text, mixed_value text, spaced_value text
) SERVER sqlite OPTIONS (
    schema '', table 'OdbcFdwMixedTable', upper_value 'UPPER_VALUE',
    lower_value 'lower_value', mixed_value 'MixedValue', spaced_value 'Spaced Value'
);

CREATE FOREIGN TABLE probe.query_types (id integer, text_value text)
SERVER sqlite OPTIONS (
    schema '', table 'query_types',
...
IMPORT FOREIGN SCHEMA ignored
    ...
    FROM SERVER sqlite INTO imported OPTIONS (schema '');

CREATE SCHEMA imported_case;
IMPORT FOREIGN SCHEMA ignored
    LIMIT TO ("odbc_fdw_lower_table", "OdbcFdwMixedTable")
    FROM SERVER sqlite INTO imported_case OPTIONS (schema '');

SELECT 'import_metadata=' || CASE WHEN
    (SELECT string_agg(
         attname || ':' || format_type(atttypid, atttypmod), ',' ORDER BY attnum)
     FROM pg_attribute
     WHERE attrelid = 'imported."ODBC_FDW_DATA_TYPES"'::regclass
       AND attnum > 0 AND NOT attisdropped) =
    'ID:integer,TEXT_VALUE:text,INTEGER_VALUE:integer,DECIMAL_VALUE:double precision,DATE_VALUE:date,TIME_VALUE:time without time zone,TIMESTAMP_VALUE:timestamp without time zone,NULL_VALUE:text,UNICODE_VALUE:text'
...
SELECT 'direct_temporal=' || CASE WHEN
    ...
    (SELECT time_value FROM probe.direct_types WHERE id = 1) = TIME '03:04:05' AND
...
SELECT 'type_matrix=' || CASE WHEN
    ...
    (SELECT decimal_value FROM probe.type_matrix WHERE id = 1) = 12345.6789 AND
...
SELECT 'type_matrix_nulls=' || CASE WHEN
    (SELECT tiny_value IS NULL AND small_value IS NULL AND integer_value IS NULL
            ...
            AND double_value IS NULL AND boolean_value IS NULL AND char_value IS NULL
            AND varchar_value IS NULL AND text_value IS NULL AND date_value IS NULL
            AND time_value IS NULL AND timestamp_value IS NULL AND blob_value IS NULL
